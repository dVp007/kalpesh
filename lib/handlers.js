...
//getAll-products
handlers.productGetall = function(d,callback){
    var acceptableMethods = ['get'];
    var data = [];
    if(acceptableMethods.indexOf(d.method)>-1){
        var files = _data.getFiles('products');
        console.log(files);
...
//getAll-beacon
handlers.beaconsGetall = function(d,callback){
    var acceptableMethods = ['get'];
    var data = [];
    if(acceptableMethods.indexOf(d.method)>-1){
        var files = _data.getFiles('beacons');
        //listing all files using forEach
        files.forEach(function (file) {
            var s;
            // Do whatever you want to do with the file
            s = file.split('.')[0];
            data += _data.read('beacons',s)+".";
        });
    return_data = data.split(".");
    callback(200,{return_data});    
    }else{
        ...
        if(beaconId){  
            ...
            //Make sure the token id valid
                ...
                for(value of beaconData.productIds){
                    data += _data.read('products',value)+"."
                }
                return_data = data.split(".")
                callback(200,{return_data}); 
        }else{
